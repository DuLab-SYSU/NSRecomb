...
subjects_order = ['SARS-CoV-2', 'RmYN02', 'RaTG13',
                  'PangolinGD', 'PangolinGX', 'BatSL', 'SARSr-CoVs']
...
def read_data(_in, **kwargs):
    ...
    x_y = [(int(k), sum([float(v_i[2]) for v_i in v]) / len(v))
           for k, v in contents.items()]
...
def merge_cds(task):
    ...
    offset = 0
    for cds_i in cds:
        fragments_by_virus, x_y = read_data(
            "/home/zeng/python_work/nCov/old_version/recombination_task/task_yn02/%s/%s_%s.501.json" % (task, task, cds_i), inter_length=3, min_length=10)
        print("CDS:", cds_i)
        pprint.pprint(fragments_by_virus)
        x_y = sorted(x_y, key=lambda x: x[0])
        length = x_y[-1][0]
        x_y_merged.extend([(x_+offset, y_) for x_, y_ in x_y])
        def f(x): return (x[0]+offset, x[1]+offset, x[2])
...
def cds(ax, start, length, y, y2, label):
    ...
    ax.arrow(start, y, length-80, 0, head_width=0.02,
...
def plot_panel(tasks=['SARS-CoV-2', 'RmYN02', 'RaTG13', 'PangolinGD', 'PangolinGX'],
    ...
    fig, axes = plt.subplots(
        n_row+1, 1, figsize=(5.5, 4), sharex=True, dpi=300)
    ...
    for i, task in enumerate(tasks):
        ...
        x_, y_ = zip(*x_y)
        # bootstrap plot
        axes[i+1].plot(x_, y_, alpha=0.8, lw=0.3,
                       color='#808080', antialiased=True)
        # threshod line 0.8
        ...
        axes[i+1].text(0.1, 0.1, "Query: %s, Backbone: %s" %
                       (task, backbones[i]), fontsize=8, font='Arial')
