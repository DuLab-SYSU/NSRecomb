...
def multiseqalign(query_i, backbone_i, sbjct_i):
    ...
    other_seq.close()
    seqs = list(msa)
    for seq, input_ in zip(seqs, [query_i, backbone_i, sbjct_i]):
        seq.annotations = input_.annotations
    return seqs
...
if __name__ == "__main__":
    ...
    MAFFT_PATH = '../package/mafft-linux64'
    KAKS_PATH = '../package/KaKs_Calculator2.0/bin'

    query = SeqIO.read('../example/RmYN02.S.fasta', 'fasta')
    backbone = BlastTools.get_gene_backbone(query, db='../db/all_cov_unify', negative_seqidlist='../example/RmYN02.acc')

    query_i = query[1002: 1503]
    backbone_i = BlastTools.get_window_backbone(query_i, backbone)
    hits = BlastTools.get_hits(query_i, db='../db/all-cov', negative_seqidlist='../example/RmYN02.acc', qcov_hsp_perc=70)
    ...
    print('BootStrap Value: ', bootstrap(*align_seqs))
